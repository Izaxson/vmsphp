...
<html>
  ...
  <body class="hold-transition login-page">
	...
	<div class="login-logo">
	  ...
	  <img src="Realtripslogo.jpg" height='140' width='150' alt="..." class="img-circle profile_img"><br> <br>
       </div><!-- /.login-logo -->
      <div class="login-logo">
	  <img src="Capture.PNG" alt="sgs logo" width="350" height="100"><br>
       </div><!-- /.login-logo -->
	   <button type="submit" class="btn btn-warning btn-block btn-flat" name="login"<a href="vehiclelog/index.php"</a> Auditor  LogIn</button>
		   ...
		   <!-- <div class="form-group has-feedback">
             <select class="form-control select2" style="width:100%" name="Role" required>
				...
				include('dist/includes/dbcon.php');
                   ...
                   $query4=mysqli_query($con,"select * from Role order by id ASC")or die(mysqli_error($con));
                      while($row3=mysqli_fetch_array($query4)){
                ?>
                    <option value="<?php echo $row3['id'];?>"><?php echo $row3['Role_name'];?></option>
                  <?php }?>
                </select>
          </div>-->
          <div class="row">
			...
			<div class="col-xs-6 pull-right">
              <button type="submit" class="btn btn-warning btn-block btn-flat" name="login" default>Sign In</button>
            </div><!-- /.col -->
          </div>
        </form>
		<br><br>
